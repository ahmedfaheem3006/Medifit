...
export class AuthService {
  ...
  loginWithGoogle() {
    ...
    const popup = window.open('https://backendmedifit.vercel.app/auth/google', 'googleAuth', 'width=500,height=600');
  ...
  private handleAuthMessage(event: MessageEvent) {
    ...
    if (event.origin !== 'https://backendmedifit.vercel.app') return;
  ...
  getUserProfile(): Observable<any> {
    return this.http.get('https://backendmedifit.vercel.app/profile', { 
  ...
  refreshToken(): Observable<any> {
    return this.http.post('https://backendmedifit.vercel.app/auth/refresh-token', {}, {
      headers: this.getAuthHeaders()
    }).pipe(
      tap((response: any) => {
